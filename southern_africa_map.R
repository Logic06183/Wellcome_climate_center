...
southern_africa_data$FocusType[southern_africa_data$`Finance programmes` == 1 & is.na(southern_africa_data$FocusType)] <- "Finance and Programmes"
...
southern_africa_data$Shape <- ifelse(southern_africa_data$`Data Providers` == 1, 17, 16)

# Identify major partners (those with Official Partners = 1 and multiple focus areas or data providers)
southern_africa_data <- southern_africa_data %>%
  mutate(
    focus_count = Research + Policy + `Engagement, Advocacy, and Capacity Building` + `Finance programmes`,
    is_major_partner = ifelse(`Official Partners` == 1 & (focus_count > 1 | `Data Providers` == 1), TRUE, FALSE),
...
# Save the main map
ggsave("southern_africa_map_with_labels.png", main_map, width = 8, height = 6, dpi = 600)
...
# Save the main map with bounding boxes
ggsave("southern_africa_map_with_boxes.png", main_map_with_boxes, width = 8, height = 6, dpi = 600)
...
# Save the Johannesburg/Tshwane inset map
ggsave("johannesburg_tshwane_inset_map.png", jhb_tshwane_map, width = 6, height = 5, dpi = 600)
...
# Save the Cape Town inset map
ggsave("cape_town_inset_map.png", cape_town_map, width = 6, height = 5, dpi = 600)
...
combined_map <- ggdraw() +
  ...
  draw_plot(inset_map, 0.65, 0.05, 0.3, 0.3)

# Save the combined map
ggsave("southern_africa_map_with_inset.png", combined_map, width = 10, height = 8, dpi = 600)
